...
private:
	static LPDIRECT3DTEXTURE9 m_apTexture[UI_TYPE_MAX];	//テクスチャのポインタ配列
	UI_TYPE m_type;										//UIタイプ
	D3DXVECTOR3 m_pos;									//位置
	D3DXVECTOR3 m_size;									//サイズ
	D3DXVECTOR3 m_move;									//移動量
	D3DXCOLOR m_color;									//色
	int m_nCntColor;									//色の更新用カウント
	int m_nCntUi;										//UIの更新用カウント
	bool m_bLogoMove;									//ロゴの移動判定
	bool m_bTransparence;								//透明度の判定
	bool m_bPauseChoice;								//ポーズメニューの選択判定
};
