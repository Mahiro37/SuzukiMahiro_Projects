		...
		{ //SPACEキー または Aボタンを押している間
			if (m_pBullet == NULL)
			{
				...
				{ //チャージしていないとき
					...
					if (GetTexType() == NOT_INVERSION)
					{
						//弾の生成
						m_pBullet = CBullet::Create(D3DXVECTOR3(m_pos.x + 100.0f, m_pos.y + BULLET_CREATE_POINT.y, m_pos.z), m_nCntPress);
					}
					else if (GetTexType() == INVERSION)
					{
						//弾の生成
						m_pBullet = CBullet::Create(D3DXVECTOR3(m_pos.x - 100.0f, m_pos.y + BULLET_CREATE_POINT.y, m_pos.z), m_nCntPress);
					}
			...
			if (m_pBullet != NULL)
			{
				...
				{
					//弾の位置の更新
					m_pBullet->SetPos(D3DXVECTOR3(m_pos.x - BULLET_CREATE_POINT.x, m_pos.y + BULLET_CREATE_POINT.y, m_pos.z));
				}
				else if (GetTexType() == INVERSION)
				{
					//弾の位置の更新
		...
		for (int nCntScene = 0; nCntScene < MAX_OBJECT; nCntScene++)
		{
			...
			if (pEnemy != NULL)
			{
				...
				{ //ボスボールに当たったとき
					...
					if (CollisionRectangleAndCircular(m_pos, targetPos, D3DXVECTOR3(PLAYER_COLLISION_SIZE.x, PLAYER_COLLISION_SIZE.y, PLAYER_COLLISION_SIZE.z), BOSS_BALL_RADIUS, pEnemy->GetRadian()) == true)
					{
				...
				{ //ファーストボールに当たったとき
					...
					if (CollisionRectangleAndCircular(m_pos, targetPos, D3DXVECTOR3(PLAYER_COLLISION_SIZE.x, PLAYER_COLLISION_SIZE.y, PLAYER_COLLISION_SIZE.z), FIRST_BALL_RADIUS, pEnemy->GetRadian()) == true)
					{
						m_PlayerState = PLAYER_STATE_DEATH;
				...
				{ //セカンドボールに当たったとき
					...
					if (CollisionRectangleAndCircular(m_pos, targetPos, D3DXVECTOR3(PLAYER_COLLISION_SIZE.x, PLAYER_COLLISION_SIZE.y , PLAYER_COLLISION_SIZE.z), SECOND_BALL_RADIUS, pEnemy->GetRadian()) == true)
					{
						m_PlayerState = PLAYER_STATE_DEATH;
				...
				{ //サードボールに当たったとき
					...
					if (CollisionRectangleAndCircular(m_pos, targetPos, D3DXVECTOR3(PLAYER_COLLISION_SIZE.x, PLAYER_COLLISION_SIZE.y, PLAYER_COLLISION_SIZE.z), THIRD_BALL_RADIUS, pEnemy->GetRadian()) == true)
					{
						m_PlayerState = PLAYER_STATE_DEATH;
			...
			{ //アイテムに当たったとき
				...
				if (CollisionRectangleAndCircular(m_pos, targetPos, D3DXVECTOR3(PLAYER_SIZE.x, PLAYER_SIZE.y, PLAYER_SIZE.z), ITEM_RADIUS, pItem->GetRadian()) == true)
				{
					//アイテムタイプの取得
...
//チャージしているかの判定の取得
//*****************************************************************************
bool CPlayer::GetChargeRelease(void)
{
	return m_bRelease;
}