...
private:
	static LPDIRECT3DTEXTURE9 m_pTexture[TYPE_MAX];	//テクスチャのポインタ配列
	ENEMY_STATE m_EnemyState;						//敵の状態
	D3DXVECTOR3 m_BarrierPosOld;					//古い位置
	D3DXVECTOR3 m_move;								//移動量
	int m_nBarrierLife;								//バリアのライフ
	int m_nBallLife;								//敵のライフ
	int m_nDamageColorChange;						//ダメージで色を変える時間
	int m_nCntCreateBall;							//ボスボールが雑魚を生成するカウント
};
