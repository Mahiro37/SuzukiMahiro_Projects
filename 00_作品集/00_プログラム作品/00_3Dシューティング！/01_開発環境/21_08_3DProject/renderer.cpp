...
HRESULT CRenderer::Init(HWND hWnd, bool bWindow)
{
	...
	d3dpp.BackBufferWidth = (UINT)SCREEN_SIZE.x;				// ゲーム画面サイズ(幅)
	d3dpp.BackBufferHeight = (UINT)SCREEN_SIZE.y;				// ゲーム画面サイズ(高さ)
...
{
	CScene::OBJTYPE UpdateTypeList[CScene::OBJTYPE_MAX] = {};
	CScene::GetUpdateType(UpdateTypeList);

	if (UpdateTypeList[0] == CScene::OBJTYPE_MAX)
	{
		//全てのシーンの更新処理
		CScene::UpdateAll();
	}
	...
	{
		//選択したシーンオブジェクトのみの更新処理
		CScene::DesignationUpdateAll(UpdateTypeList);
	}
}
...
{
	RECT rect = { 0, 0, (LONG)SCREEN_SIZE.x, (LONG)SCREEN_SIZE.y };
	char str[256];
