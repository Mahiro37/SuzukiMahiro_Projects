...
HRESULT CWallData::Init(void)
{
	...
	// ファイルを開く
	pFile = fopen("Data/Text/04_wall.txt", "r");
	...
	if (pFile != NULL)
	{
		...
		if (strcmp(cHeadText, "SCRIPT") == 0)
		{
			...
			while (strcmp(cHeadText, "END_SCRIPT") != 0)
			{
				...
				// cHeadTextがWALL_DATASETの時
				if (strcmp(cHeadText, "WALL_DATASET") == 0 && m_nCreateNum < WALL_MAX)
				{
					// cHeadTextがEND_WALL_DATASETになるまで
					while (strcmp(cHeadText, "END_WALL_DATASET") != 0)
					{
						fgets(cReadText, sizeof(cReadText), pFile);
						sscanf(cReadText, "%s", &cHeadText);
						...
						{
							sscanf(cReadText, "%s %s %d", &cDie, &cDie, &m_aCreateInfo[m_nCreateNum].nType);
						}
